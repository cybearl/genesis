...
export default function Layout({
    ...
    const { appLoadingStatus, environment, preferences } = useContext(CoreContext);

    useInterval(async () => {
        const res = await window.ipcBridge("/api/sys-info");
        setSysInfo(res.data as SysInfo);
    }, preferences?.sysInfo.refreshInterval || 1500);
    ...
    return (
        <div className="z-0 relative w-full h-screen min-h-screen overflow-hidden flex flex-col">
            ...
            {(appLoadingStatus?.currentWindow === "splash-screen" || !appLoadingStatus?.loaded) ? (
                <SplashScreen
                    isEnabled={true}
                />
            ) : (
